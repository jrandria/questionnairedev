...
class Auth extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->database();//Fait déjà par autoload
	...
	function login()
	{
		...
		//validation des form input
		$this->form_validation->set_rules('identity', 'Identity', 'required');
		$this->form_validation->set_rules('password', 'Password', 'required');

		if ($this->form_validation->run() == true)
		{
			// On test si l'utilisateur est bien identifié
			// On vérifie si on a coché "remember me"
			$remember = (bool) $this->input->post('remember');

			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				//Si l'identification s'est bien passé (succès)
				//on rédirige vers la page accueil 
			...
			else
			{
				...
				redirect('login', 'refresh');//redirect('auth/login', 'refresh'); // indication du librairie (use redirects instead of loading views for compatibility with MY_Controller libraries)
			}
		}
		else
		{
	...
	function forgot_password()
	{
		...
		else
		{
			...
			if(empty($identity)) {
		                ...
		                $this->session->set_flashdata('message', $this->ion_auth->errors());
                		redirect("forgot_password.html", 'refresh');//redirect("auth/forgot_password", 'refresh');
            		}
			...
			if ($forgotten)
			{
				...
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("login", 'refresh');//redirect("auth/login", 'refresh'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("forgot_password", 'refresh');//redirect("auth/forgot_password", 'refresh');
			}
		}
	}
